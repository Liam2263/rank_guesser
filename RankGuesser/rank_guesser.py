...
class RankGuesser(BaseScript, GameTickPacket):
    ...
    def render_text(self, text: str):
        ...
        self.renderer.draw_string_2d(20, 20, 5, 5, text, self.renderer.cyan())
        ...
        while True:
            ...
            if self.total_cars > 6:
                ...
                if Op_team_bot == "Necto":
                    ...
                    if True:
                        if diff <= -70:
                            self.render_text("Bronze")
                        elif diff <= -50 and diff >= -69:
                            self.render_text("Silver")
                        elif diff <= -30 and diff >= -49:
                            self.render_text("Gold")
                        elif diff <= -11 and diff >= -29:
                            self.render_text("Platinum")
                        elif diff <= 10 and diff >= -10:
                            self.render_text("Diamond")
                        elif diff <= 25 and diff >= 11:
                            self.render_text("Champ")
                        elif diff <= 40 and diff >= 26:
                            self.render_text("Grand Champion")
                        elif diff >= 41:
                            self.render_text("SuperSonicLegend (SSL)")
