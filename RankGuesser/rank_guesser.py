...
class RankGuesser(BaseScript, GameTickPacket):
    ...
    def searchteam(self, team):
        ...
        if len(team) == 1:
            ...
            # 2s
            if team[0].name == "Necto" and team[1].name == "Necto":
                return [True, "Necto"]
            elif team[0].name == "Nexto" and team[1].name == "Nexto":
                return [True, "Nexto"]
            elif team[0].name == "Botimus Prime" and team[1].name == "Botimus Prime":
                return [True, "Botimus Prime"]
            return [False, "Nothing"]
        elif len(team) == 3:
            # 3s
            if team[0].name == "Necto" and team[1].name == "Necto" and team[2].name == "Necto":
                return [True, "Necto"]
            elif team[0].name == "Nexto" and team[1].name == "Nexto" and team[2].name == "Nexto":
                return [True, "Nexto"]
            elif team[0].name == "Botimus Prime" and team[1].name == "Botimus Prime" and team[2].name == "Botimus Prime":
                return [True, "Botimus Prime"]
            return [False, "Nothing"]
    ...
    def run(self):
        while True:
            ...
            print(self.blue_team[0].name)
            if len(self.blue_team) == len(self.orange_team) and len(self.blue_team) <= 3:
                self.validtotalcars = True
            else:
                self.validtotalcars = False
            self.blue_team_search = self.searchteam(self.blue_team)
            self.orange_team_search = self.searchteam(self.orange_team)

            if self.validtotalcars == True:
                # Error 101
                print("MatchMaker ERROR: Teams have different values of cars or greater than 3 players")
                return
            else:
                Op_team = None
                Op_team_bot = "None"
                ...
                if self.blue_team_search[0] == True and self.orange_team_search[0] == False:
                    ...
                    # Error 201
                    print("MatchMaker ERROR: Contact SoldierDoge")
                    return
